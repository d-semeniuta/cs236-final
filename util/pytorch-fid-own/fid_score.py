...
EDITS MADE BY DANIEL SEMENIUTA (d-semeniuta)
Added ability to unpack PyTorch datasets for the sake of computing FID scores
"""
import os, sys
import pathlib
...
def unpack_dataset(dataset, path):
    if os.path.exists(path):
        print('No need to unpack')
        return
    ...
    for i, (img, label) in enumerate(tqdm(dataloader)):
        class_out_loc = os.path.join(path, str(label.item()))
        out_file = os.path.join(class_out_loc, '{}.png'.format(i))
        save_image(img.squeeze(0), out_file)
...
if __name__ == '__main__':
    ...
    if len(dataset) > 0:
        unpack_dataset(dataset, paths[1])
